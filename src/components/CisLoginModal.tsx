...
export const generateBookmarkletCode = (targetUrl: string): string => {
  const scriptBody = String.raw`(function(){void(async function(){try{if(!location.hostname.includes("cis.ncu.edu.tw")){alert("請先在瀏覽器開啟並登入中大課務系統 (cis.ncu.edu.tw)，再點擊此書籤！");return;}var courses=[];var seen={};function add(c){var k=(c.classNo||"")+"-"+(c.name||"");if(k!=="-"&&!seen[k]){seen[k]=true;courses.push(c);}}function parseRows(d){var trs=d.querySelectorAll("tr");for(var i=0;i<trs.length;i++){var tds=trs[i].querySelectorAll("td,th");if(tds.length>=6){var cNo=(tds[2]&&tds[2].textContent||"").trim();if(/^[A-Z]{2,}\d+/i.test(cNo)){var sNo=(tds[1]&&tds[1].textContent||"").trim();var rawName=(tds[4]&&tds[4].textContent||"").trim();var cName=rawName.split(/\s+/)[0]||rawName;var cr=Number((tds[6]&&tds[6].textContent||"").trim())||0;var st=(tds[tds.length-1]&&tds[tds.length-1].textContent||"").trim();add({serialNo:sNo,classNo:cNo,name:cName,credit:cr,status:st||"passed"});}}}}var xmlUrls=["/Course/main/support/sheets.xml","/Course/main/support/course.xml?id=my_class"];await Promise.all(xmlUrls.map(async function(u){try{var xRes=await fetch(u);if(xRes.ok){var xText=await xRes.text();if(!xText.includes("window.location")){var xDoc=new DOMParser().parseFromString(xText,"text/xml");var xCourses=xDoc.querySelectorAll("Courses > Course");for(var y=0;y<xCourses.length;y++){var el=xCourses[y];add({serialNo:el.getAttribute("SerialNo")||"",classNo:el.getAttribute("ClassNo")||"",name:el.getAttribute("Title")||"",credit:Number(el.getAttribute("credit")||0),status:el.getAttribute("Status")||""});}}}}catch(_){}}));try{var resStatus=await fetch("/Course/main/personal/perCrsstatus");if(resStatus.ok){var html=await resStatus.text();if(!html.includes("window.location")&&!html.includes("閒置時間過長")){var doc=new DOMParser().parseFromString(html,"text/html");parseRows(doc);var opts=doc.querySelectorAll("select[name='semester'] option");var sems=[];for(var j=0;j<opts.length;j++){var val=opts[j].value;if(/^\d{4}$/.test(val))sems.push(val);}await Promise.all(sems.map(async function(sVal){try{var sRes=await fetch("/Course/main/personal/perCrsstatus",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:"semester="+encodeURIComponent(sVal)});if(sRes.ok){var sHtml=await sRes.text();parseRows(new DOMParser().parseFromString(sHtml,"text/html"));}}catch(_){}}));}}}catch(_){}if(courses.length===0){alert("課務系統中尚未查詢到修課紀錄（歷年修課狀況表與本學期選課均為空）。若您為新生尚未選課，可直接在 CIM-Life 頁手動勾選課程！");return;}var payload=encodeURIComponent(JSON.stringify(courses));location.href="${targetUrl}"+String.fromCodePoint(35)+"cis_data="+payload;}catch(err){alert("同步發生錯誤："+err.message);}})();})()`;
